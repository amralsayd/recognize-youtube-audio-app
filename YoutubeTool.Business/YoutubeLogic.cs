...
namespace YoutubeTool.Business
{
    public class YoutubeLogic
    {
        ...
        public async Task<AudioFileViewModel> DownloadAudioFile(string videoId)
        {
            ...
            var ext = streamInfo.Container.GetFileExtension();
            await client.DownloadMediaStreamAsync(streamInfo, baseDir+ "\\Downloads\\Audio\\file_" + videoId + "." + ext);

            return new AudioFileViewModel()
            {
                ext = ext,
                fileName = "file_" + videoId + "." + ext,
                filePath = baseDir + "\\Downloads\\Audio\\file_" + videoId + "." + ext
            };
